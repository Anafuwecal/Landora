<template>
  <footer class="bg-landora-dark text-white">
    <!-- Main Footer -->
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        <!-- Brand Column -->
        <div>
          <RouterLink to="/" class="inline-block mb-6">
            <span class="text-3xl font-heading font-bold tracking-wider text-white">
              LANDORA
            </span>
          </RouterLink>
          <p class="text-gray-400 mb-6">
            Luxury real estate consulting and premium property solutions for 
            discerning clients seeking exceptional living spaces.
          </p>
          <div class="flex space-x-4">
            <a
              v-for="social in SOCIAL_LINKS"
              :key="social.name"
              :href="social.url"
              target="_blank"
              class="w-10 h-10 rounded-full bg-white/10 flex items-center justify-center hover:bg-landora-primary transition-colors"
              :aria-label="social.name"
            >
              <!-- Facebook -->
              <svg v-if="social.icon === 'facebook'" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M9 8h-3v4h3v12h5v-12h3.642l.358-4h-4v-1.667c0-.955.192-1.333 1.115-1.333h2.885v-5h-3.808c-3.596 0-5.192 1.583-5.192 4.615v3.385z"/>
              </svg>
              <!-- Instagram -->
              <svg v-else-if="social.icon === 'instagram'" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z"/>
              </svg>
              <!-- LinkedIn -->
              <svg v-else-if="social.icon === 'linkedin'" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
              </svg>
              <!-- Twitter -->
              <svg v-else-if="social.icon === 'twitter'" class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z"/>
              </svg>
            </a>
          </div>
        </div>

        <!-- Quick Links -->
        <div>
          <h4 class="text-lg font-heading font-semibold mb-6 uppercase tracking-wider">
            Quick Links
          </h4>
          <ul class="space-y-3">
            <li v-for="link in quickLinks" :key="link.path">
              <RouterLink
                :to="link.path"
                class="text-gray-400 hover:text-landora-accent transition-colors"
              >
                {{ link.label }}
              </RouterLink>
            </li>
          </ul>
        </div>

        <!-- Apartments -->
        <div>
          <h4 class="text-lg font-heading font-semibold mb-6 uppercase tracking-wider">
            Apartments
          </h4>
          <ul class="space-y-3">
            <li>
              <RouterLink
                to="/apartments/one-room"
                class="text-gray-400 hover:text-landora-accent transition-colors"
              >
                1 Room 47m²
              </RouterLink>
            </li>
            <li>
              <RouterLink
                to="/apartments/two-rooms"
                class="text-gray-400 hover:text-landora-accent transition-colors"
              >
                2 Rooms 65m²
              </RouterLink>
            </li>
            <li>
              <RouterLink
                to="/apartments/three-rooms"
                class="text-gray-400 hover:text-landora-accent transition-colors"
              >
                3 Rooms 90m²
              </RouterLink>
            </li>
          </ul>
        </div>

        <!-- Contact Info -->
        <div>
          <h4 class="text-lg font-heading font-semibold mb-6 uppercase tracking-wider">
            Contact Us
          </h4>
          <ul class="space-y-4">
            <li class="flex items-start">
              <MapPinIcon class="w-5 h-5 text-landora-accent mt-1 mr-3 flex-shrink-0" />
              <span class="text-gray-400">{{ ADDRESS }}</span>
            </li>
            <li>
              <a
                :href="`tel:${CONTACT_PHONE}`"
                class="flex items-center text-gray-400 hover:text-landora-accent transition-colors"
              >
                <PhoneIcon class="w-5 h-5 text-landora-accent mr-3" />
                {{ CONTACT_PHONE }}
              </a>
            </li>
            <li>
              <a
                :href="`mailto:${CONTACT_EMAIL}`"
                class="flex items-center text-gray-400 hover:text-landora-accent transition-colors"
              >
                <EnvelopeIcon class="w-5 h-5 text-landora-accent mr-3" />
                {{ CONTACT_EMAIL }}
              </a>
            </li>
          </ul>
        </div>
      </div>
    </div>

    <!-- Newsletter Section -->
<div class="border-t border-white/10">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="flex flex-col lg:flex-row items-center justify-between gap-6">
      <div class="text-center lg:text-left">
        <h5 class="text-lg font-heading font-semibold mb-1">
          Subscribe to Our Newsletter
        </h5>
        <p class="text-gray-400 text-sm">
          Get the latest updates on new properties and exclusive offers.
        </p>
      </div>
      
      <!-- Responsive Form -->
      <form @submit.prevent="handleSubscribe" class="w-full sm:w-auto">
        <!-- Stacked on mobile, inline on sm+ -->
        <div class="flex flex-col sm:flex-row gap-3 sm:gap-0">
          <input
            v-model="email"
            type="email"
            placeholder="Enter your email"
            class="w-full sm:w-64 md:w-72 px-4 py-3 bg-white/10 border border-white/20 
                   rounded-sm sm:rounded-l-sm sm:rounded-r-none
                   text-white placeholder-gray-400 
                   focus:outline-none focus:border-landora-accent
                   text-base"
            required
          />
          <button
            type="submit"
            class="w-full sm:w-auto px-6 py-3 bg-landora-primary text-white 
                   font-heading font-semibold uppercase text-sm tracking-wider 
                   rounded-sm sm:rounded-l-none sm:rounded-r-sm
                   hover:bg-landora-secondary transition-colors
                   whitespace-nowrap"
          >
            Subscribe
          </button>
        </div>
      </form>
    </div>
  </div>
</div>

    <!-- Copyright -->
    <div class="bg-black/20">
      <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div class="flex flex-col md:flex-row items-center justify-between text-sm text-gray-400">
          <p>© {{ currentYear }} LANDORA. All rights reserved.</p>
          <div class="flex space-x-6 mt-4 md:mt-0">
            <RouterLink to="/privacy" class="hover:text-landora-accent transition-colors">
              Privacy Policy
            </RouterLink>
            <RouterLink to="/terms" class="hover:text-landora-accent transition-colors">
              Terms of Service
            </RouterLink>
          </div>
        </div>
      </div>
    </div>
  </footer>
</template>

<script setup lang="ts">
import { ref, computed } from 'vue'
import { useToast } from 'vue-toastification'
import { CONTACT_PHONE, CONTACT_EMAIL, ADDRESS, SOCIAL_LINKS } from '@/utils/constants'
import { MapPinIcon, PhoneIcon, EnvelopeIcon } from '@heroicons/vue/24/outline'
import { supabase } from '@/lib/supabase'

const toast = useToast()
const email = ref('')
const currentYear = computed(() => new Date().getFullYear())

const quickLinks = [
  { label: 'About Us', path: '/about/landora' },
  { label: 'Our Properties', path: '/apartments' },
  { label: 'Facilities', path: '/facilities' },
  { label: 'News & Updates', path: '/news' },
  { label: 'Contact', path: '/contact' },
  { label: 'FAQ', path: '/pages/faq' },
]

const handleSubscribe = async () => {
  try {
    const { error } = await supabase
      .from('Subscriber')
      .insert([{ email: email.value }])

    if (error) {
      throw error
    }

    toast.success('Thank you for subscribing!')
    email.value = ''
  } catch (e: any) {
    if (e.code === '23505') {
      toast.info('You are already subscribed!')
    } else {
      toast.error('Something went wrong. Please try again.')
    }
  }
}
</script>